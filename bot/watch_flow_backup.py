                    ...
                    if search_results:
                        ...
                        try:
                            ...
                            ai_search_result = await search_products_with_ai_analysis(
                                ...
                                item_count=min(len(search_results), 10),  # Get more products for AI selection
                            ...
                            if ai_products:
                                ...
                                log.warning("AI_SEARCH: No AI-enhanced products, using original search results")
                                
                        except Exception as e:
                            log.warning(f"AI_SEARCH: Failed to get AI-enhanced products: {e}, using original search")
                        
                        # STEP 2: Apply existing filters
                        filtered_products = search_results  # Already filtered above
                        
                        if not filtered_products:
                            # This should not happen since we already checked, but safety first
                            log.warning("No products after AI enhancement")
...
async def smart_product_selection_with_ai(
    ...
    This function integrates with the AI feature matching system to provide intelligent
    product selection and determines whether to show single or multi-card experience.
    
    Args:
    ----
        products: List of product candidates
        user_query: User's search query
        user_preferences: User preferences from watch data
        enable_multi_card: Whether to enable multi-card experience
        
    Returns:
    -------
        Dict containing selection result with type and products
    """
...
async def _finalize_watch_fallback(
    ...
    This maintains the original watch creation logic to ensure 100% reliability.
    """
    from .product_selection_models import smart_product_selection
    from .models import User, Watch
    from sqlmodel import Session, select
