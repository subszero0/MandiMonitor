...
def sample_data(in_memory_db):
    ...
    with Session(in_memory_db) as session:
        ...
        watch1 = Watch(id=1, user_id=1, keywords="Samsung monitor", asin="B123456789")
        watch2 = Watch(id=2, user_id=1, keywords="Gaming laptop", asin="B987654321")
        session.add(watch1)
        session.add(watch2)

        # Create price history with different sources
        price1 = Price(
            id=1, watch_id=1, asin="B123456789", price=2500000, source="paapi"
        )
        price2 = Price(
            id=2, watch_id=1, asin="B123456789", price=2400000, source="scraper"
        )
        price3 = Price(
            id=3, watch_id=2, asin="B987654321", price=8500000, source="paapi"
        )
        session.add(price1)
        session.add(price2)
        session.add(price3)
...
def test_admin_endpoint_with_correct_auth(
    client: FlaskClient, auth_headers: dict, sample_data
):
...
def test_prices_csv_with_auth(client: FlaskClient, auth_headers: dict, sample_data):
